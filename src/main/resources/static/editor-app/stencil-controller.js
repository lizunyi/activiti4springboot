...
angular.module('activitiModeler')
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
        ...
        $scope.editorFactory.promise.then(function () {
            ...
            //lizunyi 添加个生成UUID的方法
            $scope.uuid = function(){
                var d = new Date().getTime();
                if (window.performance && typeof window.performance.now === "function") {
                    d += performance.now(); //use high-precision timer if available
                }
                var uuid = 'yxxxxxxx'.replace(/[y]/g,function(){
            		return String.fromCharCode(97 + Math.floor(Math.random() * Math.pow(10,16)) % 26);
                ...
                if (shapes && shapes.length > 0) {
                    ...
                    for (var i = 0; i < properties.length; i++) {
                        ...
                        if (propertyConfig === undefined || propertyConfig === null) {
                            ...
                            //lizunyi 添加默认元素节点的属性值
                            if(selectedShape.properties.hasOwnProperty("oryx-process_id") && (!selectedShape.properties["oryx-process_id"] || selectedShape.properties["oryx-process_id"] == "未定义")){
                            	var modelId = EDITOR.UTIL.getParameterByName('modelId'); 
                            	jQuery.ajax({
                        			url:KISBPM.URL.getModelEntity(modelId),
                        			async:false,
                        			success:function(data){
                        				selectedShape.properties["oryx-process_id"] = data.key;
                                      	selectedShape.properties["oryx-name"] = data.name;
                        			}
                        		});
                            }

                            var $groupName = stencil._jsonStencil.groups[0];
                            if($groupName == "开始事件" || $groupName == "结束事件" || $groupName == "活动"){
	                            if(!selectedShape.properties[key]){
	                                if ("oryx-overrideid" == key){
		                            	 if("开始事件" == $groupName){
		                                 	selectedShape.properties[key] = "start";
		                                 }else if("结束事件" == $groupName){
		                                 	selectedShape.properties[key] = "end";
		                                 }else{
		                                	 selectedShape.properties[key] = $scope.uuid();
		                                 }
		                            }else if( "oryx-name" == key){
		                            	 if("开始事件" == $groupName){
		                                 	selectedShape.properties[key] = "开始";
		                                 }else if("结束事件" == $groupName){
		                                 	selectedShape.properties[key] = "结束";
		                                 }else{
		                                	 selectedShape.properties[key] = selectedShape.properties["oryx-overrideid"];
		                                 }
		                            } 
	                            }
                            }
