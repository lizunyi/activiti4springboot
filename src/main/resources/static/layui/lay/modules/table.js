...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  ,ELEM_VIEW = 'layui-table-view', ELEM_TOOL = '.layui-table-tool', ELEM_BOX = '.layui-table-box', ELEM_INIT = '.layui-table-init', ELEM_HEADER = '.layui-table-header', ELEM_BODY = '.layui-table-body', ELEM_MAIN = '.layui-table-main', ELEM_FIXED = '.layui-table-fixed', ELEM_FIXL = '.layui-table-fixed-l', ELEM_FIXR = '.layui-table-fixed-r', ELEM_TOTAL = '.layui-table-total', ELEM_ALL_TOTAL = '.layui-table-all-total', ELEM_PAGE = '.layui-table-page', ELEM_SORT = '.layui-table-sort', ELEM_EDIT = 'layui-table-edit', ELEM_HOVER = 'layui-table-hover'
    ...
    return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
      ...
      ,'{{# layui.each(d.data.cols, function(i1, item1){ }}'
        ...
        ,'{{# layui.each(item1, function(i2, item2){ }}'
          ...
          ,'{{# if(item2.fnHide){ item2.hide = item2.fnHide(item2); } }}'
          ,'{{# var isSort = !(item2.colGroup) && item2.sort; }}'
          ,'<th data-field="{{ item2.field||i2 }}" data-key="{{d.index}}-{{i1}}-{{i2}}" {{# if( item2.parentKey){ }}data-parentkey="{{ item2.parentKey }}"{{# } }} {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize || item2.colGroup){ }}data-unresize="true"{{# } }} class="{{# if(item2.hide){ }}layui-hide{{# } }}{{# if(isSort){ }} layui-unselect{{# } }}{{# if(!item2.field){ }} layui-table-col-special{{# } }}" {{# if(item2.style){ }}style="{{item2.style}}" {{# } }}>'
  ...
  ,TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
    ...
    ,'{{# if(d.data.allTotalRow){ }}'
      ,'<div class="layui-table-total layui-table-all-total">'
        ,'<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
        ,'{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>'
          ,'<tbody><tr><td><div class="layui-table-cell" style="visibility: hidden;">Total</div></td></tr></tbody>'
      , '</table>'
  ...
  Class.prototype.render = function(){
    ...
    //数据总计
    that.layAllTotal = reElem.find(ELEM_ALL_TOTAL);
      ...
      $.ajax({
        ...
        ,success: function(res){
          ...
          if(typeof options.parseData === 'function'){
            res = options.parseData(res,that) || res;
          }
          //检查数据格式是否符合规范
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
	...
	options.beforeRenderAllTotal && options.beforeRenderAllTotal.call(this);
    that.renderAllTotal(data); //数据合计
...
//数据总计行
  Class.prototype.renderAllTotal = function(data, totalRowData){
    var that = this
    ,options = that.config;
    
    if(!options.allTotalRow) return;
    
    var tds = [];
    that.eachCols(function(i3, item3){
      var field = item3.field || i3;
      let content="";
	  if(item3.allTotalRowText){
		  content = item3.allTotalRowText;
	  }else{
		  if(item3.allTotalRow){
			  content = that.dataAllTotal[field] || "0.00";
		  }
	  }
    ...
    that.layAllTotal.find('tbody').html('<tr>' + tds.join('') + '</tr>');
  };
  ...
  Class.prototype.fullSize = function(){
    ...
    //添加总计行数据
	if(options.allTotalRow){
      bodyHeight = bodyHeight - (that.layAllTotal.outerHeight() || 40);
    }
  ...
  Class.prototype.scrollPatch = function(){
    ...
    addPatch(that.layAllTotal);// 添加补丁
